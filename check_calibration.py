...
import pickle
from utils.utils import get_mean_std_truth, uq_label_plot
from utils.calibration import calibrated_regression

# only used for checking calibration for some datasets!

def main():
    calibration_type = 'all'
    test_name = 'CARD_holdout_seed_42_Exp_6_inference_result_.csv'
    val_name = 'CARD_holdout_seed_42_Exp_6_inference_result_validation_.csv'
    dataset_name = 'BPIC20TPD'
    root_path = os.getcwd()
    dataset_path = os.path.join(root_path, 'results', dataset_name, 'dalstm')
    test_path = os.path.join(dataset_path, test_name)
    val_path = os.path.join(dataset_path, val_name) 
    recalibration_path = os.path.join(dataset_path, 'recalibration')       
    ...
    uq_eval(calibrated_result, 'CARD', report=True, verbose=True,
            calibration_mode=True, calibration_type=calibration_type,
...
def uq_eval(csv_file, prefix, report=False, verbose=False,
    ...
    if report:        
        ...
        # Note that this plot is not affected by calibration, since CR simply 
        ...
        if not calibration_mode:
            ...
            if (prefix=='DA+H' or prefix=='CDA+H' or prefix == 'DE+H' or 
                prefix == 'BE+H' or prefix == 'LA' or prefix=='deterministic' or 
            ...
            if (prefix=='DA+H' or prefix=='CDA+H' or 
                prefix == 'DE+H' or prefix == 'BE+H'  or prefix == 'LA' or
